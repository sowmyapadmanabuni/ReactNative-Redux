import React, { Component } from 'react';
import {
  Platform, AppRegistry, StyleSheet, Text, View, TextInput, PermissionsAndroid,Button ,
  FlatList, ActivityIndicator, TouchableOpacity, Image, Alert,
  ScrollView
} from 'react-native';
import { QRCode } from 'react-native-custom-qr-codes';

import RNHTMLtoPDF from 'react-native-html-to-pdf';
import { captureScreen } from "react-native-view-shot";
//import Share from 'react-native-share';
//import RNFS from 'react-native-fs';

var txt = '';// '<h1 style="text-align: center;"><strong>Hi suvarna</strong></h1><p style="text-align: center;">Here is an example of pdf Print in React Native</p><p style="text-align: center;"><strong>Team About React</strong></p>'+ '<h1 style="text-align: center;"><strong>Hi suvarna</strong></h1><p style="text-align: center;">Here is an example of pdf Print in React Native</p><p style="text-align: center;"><strong>Team About React</strong></p>';

export default class QRCodeGeneration extends Component {
  state = {
    filePath: 'Select',
    dataSource: [],
    isLoading: true,
    imageURI : 'http://aboutreact.com/wp-content/uploads/2018/07/sample_img.png',
    dataBase64:null,

  }
  constructor(props) {
    super(props)

    console.log('QRCodeGeneration', 'constructor');
    var that = this;
    async function requestExternalWritePermission() {
      try {
        const granted = await PermissionsAndroid.request(
          PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE,
          {
            title: 'CameraExample App External Storage Write Permission',
            message:
              'CameraExample App needs access to Storage data in your SD Card ',
          }
        );
        if (granted === PermissionsAndroid.RESULTS.GRANTED) {
          //If WRITE_EXTERNAL_STORAGE Permission is granted
          //changing the state to show Create PDF option
          that.setState({ isPermitted: true });
        } else {
         // alert('WRITE_EXTERNAL_STORAGE permission denied');
        }
      } catch (err) {
       // alert('Write permission err', err);
        console.warn(err);
      }
    }
    //Calling the External Write permission function
    requestExternalWritePermission();
  }
  static navigationOptions = {
    title: 'QRCodeGeneration CheckPointList',
    headerStyle: {
      backgroundColor: '#FA9917',
    },
    headerTintColor: 'white',
    headerTitleStyle: { color: 'white' }
  };

  async createPDF() {
    let options = {
      //Content to print
      html:
        '<h1 style="text-align: center;"><strong>Hello Guys</strong></h1><p style="text-align: center;">Here is an example of pdf Print </p>' +
        '<QRCode style={{margin:10}} value={params.name + \',\' + params.cpgpsPnt + \',\' + global.SelectedAssociationID}size={200} bgColor=\'black\' fgColor=\'white\' />' +
        '<p style="text-align: center;"><strong>Team About React</strong></p>',
      //File Name
      fileName: 'test',
      //File directory
      directory: 'docs',
    };
    let file = await RNHTMLtoPDF.convert(options);
    console.log(file.filePath);
    this.setState({ filePath: file.filePath });
  }
  takeScreenShot=()=>{
    //handler to take screnshot
    captureScreen({
      //either png or jpg or webm (Android). Defaults to png
      format: "jpg",
      //quality 0.0 - 1.0 (default). (only available on lossy formats like jpg)
      quality: 0.8
    })
    .then(
      //callback function to get the result URL of the screnshot
      uri => {this.setState({ imageURI : uri },
       )

    },
      error => console.error("Oops, Something Went Wrong", error)
    );
  //  this.uploadImage();
  /* RNFS.readFile(this.state.imageURI, "base64").then(data => {
    // binary data
    console.log('data base64 '+data);
    this.setState({ dataBase64 : data });
    let shareImageBase64 = {
      title: "React Native",
      message: "Hola mundo",
      url: 'data:image/png;base64,'+this.state.dataBase64,
      subject: "Share Link" //  for email
    };
    Share.open(shareImageBase64);
  }); */
  
  }

  uploadImage(){
    const { params } = this.props.navigation.state;

    //   alert('Guard Added Successfully!');
    console.log('WorkersLis uploadImage ', params.ChkPntID)

    const imgName = 'Association' + global.SelectedAssociationID + 'QR' + params.ChkPntID + '.jpg';
    //      String imgName = PERSON + "Association" + prefManager.getAssociationId() + GUARD + movie.getGuardID() + ".jpg";
    console.log('WorkersLis uploadImage  '+imgName);
    
    if (this.state.imageURI) {
        var data = new FormData();
        data.append('Test', { uri: this.state.imageURI, name: imgName, type: 'image/jpg' });
        const config = {
            method: 'POST',
            headers: { "X-Champ-APIKey": "1FDF86AF-94D7-4EA9-8800-5FBCCFF8E5C1", "content-type": "multipart/form-data" },
            body: data
        };
        console.log("Config", config);
        fetch(global.uploadImageURL, config).then(responseData => {
            console.log("sucess==>");
            console.log(responseData._bodyText);
            console.log(responseData);
                 alert("Image uploaded done! Image "+imgName)
        }).catch(err => {
            console.log("err==>");
            alert("Error with image upload!")
            console.log(err);
        });
    } else {
      alert("Error !")

    }
}

  render() {
    
    // const { navigate } = this.props.navigation;
    const { params } = this.props.navigation.state;
    /////""+movie.getCheckPointName()+","+movie.getGPSPoint()+","+movie.getCreatedDate()+","
    //+movie.getAssociationID()+",",
    const {
      navigate } = this.props.navigation;
    let shareImageBase64 = {
      title: "React Native",
      message: "Hola mundo",
      url: 'data:image/png;base64,'+this.state.dataBase64,
      subject: "Share Link" //  for email
    };
    return (

      <View style={{ backgroundColor: '#FFF' }}>
  
<View style={{ height: '100%' }}>

<View>
        <View style={{flexDirection:'row',}}>
                    <View style={{flex:1, marginTop:43,marginRight:0, justifyContent:'center',marginLeft:10}}>
                        <TouchableOpacity onPress={() => navigate(('CheckPointListScreen'), { cat: '' })}
                        >
                        <Image source={require('../pages/assets/images/back.png')}
                        style={{ height: 25, width: 25, margin: 5, alignSelf: 'center' }} />
                        </TouchableOpacity>
                    </View>
                    {/* <TouchableOpacity 
                        style={{paddingTop: 2, paddingRight: 2, paddingLeft: 2, flex: 1, alignItems: 'center', flexDirection: 'row',
                            paddingBottom: 2, borderColor: 'white', borderRadius: 0, borderWidth: 2, textAlign: 'center',marginTop:'6%'}}
                            onPress={() => this.props.navigation.navigate('SideMenu')}>
                        <Image source={require('../pages/assets/images/menu_button.png')}
                            style={{ height: 25, width: 25, margin: 5, alignSelf: 'center' }} />
                    </TouchableOpacity> */}
                    <View style={{ flex: 5, alignItems:'center', justifyContent:'center'}}>
                    <Image source={require('../pages/assets/images/OyespaceRebrandingLogo.png')}
                        style={{height: 40, width: 95, marginTop: 45,marginBottom:5}} />
                    </View>  
                    <View style={{flex:1,marginTop:45, marginRight:10, justifyContent:'center',}}>    
                    </View>                 
                </View> 

                <View style={{ backgroundColor: 'lightgrey', flexDirection: "row", width: '100%', height: 1, }}></View>
                <View style={{ backgroundColor: 'lightgrey', flexDirection: "row", width: '100%', height: 1, }}></View>

                <Text style={{ fontSize: 16, color: 'black', fontWeight:'bold',margin:10 }}>QR Code</Text>


          </View>
      <View >
        <ScrollView>

          <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
            <Text style={{ margin: 10, fontSize: 14 }}>{params.name}</Text>
            <Text style={{ margin: 10 }}>{params.cpgpsPnt} </Text>
            <QRCode logo={require('../pages/assets/images/Oyespaceqrcode.png')} logoSize={80} content= {params.name + ',' + params.cpgpsPnt + ',,' + global.SelectedAssociationID}
              />
            {/* <QRCode style={{ margin: 10 }} value={params.name + ',' + params.cpgpsPnt + ',,' + global.SelectedAssociationID}
              size={200} bgColor='black' fgColor='white' /> */}
            {/* <Button title="gps"  const { params } = this.props.navigation.state;
    console.log('QRCodeGeneration componentdidmount start ', params.name);
    console.log('QRCodeGeneration params',' '+params.name+' '+ +params.cpgpsPnt+' '+params.ChkPntID);
              color='orange' width='20%'
              onPress={() => navigate('Create', {user:1})}
              /> */}
           {/*  <TouchableOpacity onPress={this.createPDF.bind(this)}>
              <View>

                <Text style={styles.text}>Create PDF</Text>
              </View>rrrrrrrr
            </TouchableOpacity>
            <Text style={styles.text}>{this.state.filePath}</Text> */}
  <Button title="Share" onPress={this.takeScreenShot} />
        {/*  <Image 
          source={{uri : this.state.imageURI}} 
          style={{width: 200, height: 300, resizeMode: 'contain', marginTop: 5}} /> 
        <TouchableOpacity style={{margin:5,padding:5,backgroundColor:'red'}}
         onPress={()=>{
          Share.open(shareImageBase64);
        }}>
          <View style={styles.instructions}>
            <Text>Simple Share Image Base 64</Text>
          </View>
        </TouchableOpacity> */}
          </View>

        </ScrollView>
      </View>
      </View>
      </View>

    );
  }
}
const REACT_ICON = 'data:image/png;base64,';

const styles = StyleSheet.create({
  MainContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: '#2F4F4F',
    borderWidth: 1,
    borderColor: '#000',
  },
  text: {
    color: 'white',
    textAlign: 'center',
    fontSize: 25,
    marginTop: 16,
  },
  ImageStyle: {
    height: 150,
    width: 150,
    resizeMode: 'stretch',
  },
});
