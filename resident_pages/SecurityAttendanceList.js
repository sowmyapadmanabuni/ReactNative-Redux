...
export default class CheckBox extends Component {
  ...
  static navigationOptions = {
    title: 'Security Attendance List',
    headerStyle: {
    ...
    return (
      <View
        ...
        <View style={{ flex: 1, flexDirection: 'column' }}>
          <Text style={styles.title}>Name: {item.guardID}</Text>
          <Text style={styles.text}>Start Time : {item.startTime.substring(11, 21)}</Text>
          <Text style={styles.text}>End Time : {item.endTime.substring(11, 21)}</Text>
  ...
  makeRemoteRequest = () => {
    ...
    const url = 'http://' + global.oyeURL + '/oye247/OyeLivingApi/v1/attendance/GetAttendanceDatelist/' + global.SelectedAssociationID + '/' + this.state.dobText.toString()
    console.log(url)
    fetch(url, {
      method: 'GET',
      headers: {
