...
import {
  AppRegistry, Platform, Alert, TouchableOpacity, ScrollView, PermissionsAndroid,
  StyleSheet, Text, Image, View, FlatList, ActivityIndicator
} from 'react-native';
...
export default class IndivisualServiceProviderReport extends Component {
  ...
  constructor() {
    ...
    console.log('IndivisualServiceProviderReport worker ID', this.state.WorkerId);
  ...
  componentDidMount() {
    console.log('anu', this.props.navigation.state.params.id);
    this.makeRemoteRequest();
    ...
    fetch(url, {
      ...
      .then((responseJson) => {
        this.setState({
          ...
          dataSource: responseJson.data.visitorLog.filter(x => x.reRgVisID == this.props.navigation.state.params.id),
          isLoading: false
        })

        console.log('IndivisualServiceProviderReport ', this.state.dataSource);
      })
      .catch((error) => {
        console.log(error)
      })
