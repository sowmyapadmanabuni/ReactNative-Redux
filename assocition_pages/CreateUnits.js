...
export default class CreateUnitsPotrait extends Component {
  constructor() {
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT Distinct BlockID,BlockName FROM Blocks where AssnID=' + global.SelectedAssociationID, [], (tx, results) => {
        console.log('Results', results.rowsAffected);
        var temp = [];
        for (let i = 0; i < results.rows.length; ++i) {
          temp.push(results.rows.item(i));
          console.log('Guards name', results.rows.item(i).BlockName+results.rows.item(i).BlockID);
        }
  ...
  //  let dpOwnershipStatus = [{ value: 'Sold', }, { value: 'Unsold', }]
    ...
    let dpOccupancyStatus = [{ value: 'Sold Vacant', },{ value: 'Sold Owner Occupied', }, { value: 'Sold Tenant Occupied', },{ value: 'Unsold Vacant', },{ value: 'Unsold Owner Occupied', } ]
...
<Text style={{ fontSize: 16, color: 'black', fontWeight:'bold',justifyContent:'center',alignContent:'center',marginBottom:10 }}>    Create Unit</Text>
        ...
        <ScrollView style={{ height: '90%', paddingLeft: 20, paddingRight: 20 }}>
          ...
          <Text style={{ color: 'orange', fontSize: 16 }}>Unit Information</Text>
          ...
          <View style={{ flex: 1 }}>
                  <Picker
                    ...
                    {this.state.dataSourceGuardPkr.map((item, key) => (
                      <Picker.Item label={item.BlockName} value={item.BlockID} key={key} />)
                    )}
          ...
          {/* <View style={{ flexDirection: 'row', }}>
                            ...
                            <View style={{ flex: 0.85 }}>
                                <TextField
                                    ...
                                    maxLength={10}
                                    onChangeText={this.handlePhonenumberOwner}
                                />
       ...
       <Picker.Item label="Residentail/Commercial" value="Residentail/Commercial" />
          ...
          <TextField
            ...
            onChangeText={this.unitParkgLot}
          />
           {/* {this.state.Occupency_Status == 'Sold Owner Occupied'||this.state.Occupency_Status == 'Unsold Owner Occupied' ?
          <Text style={{height:0}}></Text>
         :
         <View>
          <Text style={{ color: 'orange', fontSize: 16 }}>Tenant Information</Text>
          <View style={{ flexDirection: 'row' }}>
          ...
          <View style={{ flexDirection: 'row', }}>
                            ...
                            <View style={{ flex: 0.85 }}>
                                <TextField
                                    ...
                                    keyboardType='numeric'
                                    maxLength={10}
                                    onChangeText={this.handlePhonenumberTenant}
                                />
...
const styles = StyleSheet.create({
  ...
  whatisthenameofyourassoc1: {
     color: '#000', marginTop: 10,  fontSize: 13, flex: 1
  },
  ...
  rectangle: {
    ...
    marginLeft: 5, marginRight: 5, marginTop: 5, borderRadius: 2, borderWidth: 1,marginBottom:50
  },
})
