...
class App extends Component {
  ...
  createAssociationPostData = () => {
    ...
    let property_Name = this.state.property_Name;
    let property_Type = this.state.property_Type;
    let pan_Number = this.state.pan_Number;
    let reg_Number = this.state.reg_Number;
    let country_a = this.state.country_a;
    let state = this.state.state;
    let city = this.state.city;
    let association_Address = this.state.association_Address;
    let pinCode = this.state.pinCode;
    let total_NumberOfBlocks = this.state.total_NumberOfBlocks;
    let total_NumberOfUnits = this.state.total_NumberOfUnits;
    let emailAssociation = this.state.emailAssociation;
    let bankName = this.state.bankName;
    let bankIFSC = this.state.bankIFSC;
    let bankAccountNumber = this.state.bankAccountNumber;
    let bankAccountType = this.state.bankAccountType;
    ...
    if (association_Name.length == 0) {
      ...
      alert("Number of blocks should not contain alphabets and special character");
      ...
      alert("Number of units should not contain alphabets and special character");
      ...
      alert("Enter valid Bank account number");
    } else if (this.state.cca2 === 'IN') {
      if (pan_Number.length === 0 && reg_Number === 0) {
                ...
                <View style={styles.container}>
                  <Card style={styles.myProfileCardsStyle}>
                    <Item stackedLabel style={styles.cardItemsStyle}>
                      ...
                      <Input
                          style={{height: hp("5%"), paddingLeft: wp('5%')}}
                          autoCorrect={false}
                          maxLength={50}
                          autoCapitalize='characters'
                          multiline={false}
                          textAlign={"justify"}
                          value={(this.state.association_Name).toUpperCase()}
                          placeholder="Enter Association Name"
                          onChangeText={(association_Name) => this.setAssociationName(association_Name)}
                      />
                    </Item>
                    <Item stackedLabel style={styles.cardItemsStyle}>
